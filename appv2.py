...
def _init_traceloop_if_enabled() -> None:
    ...
    disable_batch = _safe_bool_env("TRACELOOP_DISABLE_BATCH", False)

    try:
        Traceloop.init(app_name=app_name, disable_batch=disable_batch)
        _safe_print(f"[telemetry] Traceloop initialized app_name={app_name}")
    except Exception as ex:
...
def init_observability() -> None:
    ...
    if _is_sdk_tracer_provider(current_tp):
        ...
        _safe_print("[telemetry] created new SDK tracer provider")
    ...
    if otlp_endpoint:
        try:
            if not _has_span_processor(tracer_provider, "BatchSpanProcessor"):
                ...
                _safe_print("[telemetry] OTLP trace exporter attached")
            else:
    ...
    if _is_sdk_meter_provider(current_mp):
        ...
        if otlp_endpoint:
            try:
                ...
                _safe_print("[telemetry] OTLP metric exporter attached")
            except Exception as ex:
    ...
    STATE.tracer = trace.get_tracer("agent.tracer")
    STATE.meter = metrics.get_meter("agent.metrics")
